...
import { Box, Typography, CircularProgress, AppBar, Toolbar, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, IconButton } from '@mui/material';
  ...
  const fetchEmails = async (key: string, ns: string, f: FilterParams) => {
    ...
    try {
      const params = [
        ...
        f.fromTimestamp ? `from=${f.fromTimestamp}` : '',
        f.toTimestamp ? `to=${f.toTimestamp}` : '',
      ].filter(Boolean).join('&');
    ...
    <Box sx={{ minHeight: '100vh', background: '#f5f6fa' }}>
      ...
      {(!apiKey || !namespace) ? (
        ...
        <Box sx={{ display: 'flex', flex: 1, minHeight: 0, px: 0, py: 0 }}>
          <Box sx={{ width: 340, minWidth: 260, maxWidth: 400, height: '85vh', overflowY: 'auto', boxShadow: 2, bgcolor: '#fff', borderRadius: 3, m: 3, mb: 3, display: 'flex', flexDirection: 'column' }}>
            <EmailList
              emails={emails}
